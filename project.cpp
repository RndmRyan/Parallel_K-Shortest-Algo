...
int main(int argc, char** argv) 
{
    ...
    int world_size, rank, *TotalscaterInfo, k = 3;
    ...
    int* MyscaterInfo = new int[2];
    MPI_Scatter(TotalscaterInfo, 2, MPI_INT, MyscaterInfo, 2, MPI_INT, 0, MPI_COMM_WORLD);
    cout << "Process " << rank << " is finding shortest path from " << MyscaterInfo[0] << " to " << MyscaterInfo[1] << endl; 
    findKShortestPaths(adjMatrix, indexToNode, MyscaterInfo[0], MyscaterInfo[1], k);
    cout << endl; 

    // MPI_Gather(MyscaterInfo, 1, MPI_INT, TotalscaterInfo, 1, MPI_INT, 0, MPI_COMM_WORLD);
    //print first 10 only
